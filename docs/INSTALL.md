...
Download these assets from GitHub Release `v1.6.17`:

- `sb-gateway-1.6.17-routeros-bundle.zip`
- `sb-gateway-1.6.17-routeros-bundle.zip.sha256`

The bundle contains:

- `sb-gateway-1.6.17-linux-arm64.tar`
- `sb-gateway-1.6.17-linux-arm64.tar.sha256`
- `sb-gateway-1.6.17-linux-arm64.manifest.json`
...
```powershell
(Get-FileHash .\sb-gateway-1.6.17-routeros-bundle.zip -Algorithm SHA256).Hash
Get-Content .\sb-gateway-1.6.17-routeros-bundle.zip.sha256
```

Linux or macOS:

```sh
sha256sum -c sb-gateway-1.6.17-routeros-bundle.zip.sha256
```

After extraction, verify the container archive too:

```sh
sha256sum -c sb-gateway-1.6.17-linux-arm64.tar.sha256
```
