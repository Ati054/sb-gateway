...
Документ соответствует SB Gateway 1.6.21, серверному Xray-core 26.9.9 и runtime renderer
schema 38.
...
пути. Общая настройка `system.routing_monitor` задаёт паузу; значение по
умолчанию — 3 секунды. Один успешный HTTPS-ответ завершает проверку. Для быстрого
...
Когда selector находится в `block`, health-loop временно использует общий
интервал восстановления (по умолчанию 15 секунд) и проверяет ограниченную порцию
кандидатов. В режиме Auto это до двух кандидатов URLTest и до трёх кандидатов
приоритетного листа.
