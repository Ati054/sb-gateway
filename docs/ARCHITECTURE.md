...
умолчанию — 3 секунды. Один успешный HTTPS-ответ завершает проверку. Если
первая контрольная цель не ответила за 2 секунды, быстрая проверка запускает
две оставшиеся независимые HTTPS-цели параллельно с лимитом 3 секунды. Первый
успех отменяет соседнюю пробу; отказ отдельных сайтов не объявляет узел
неисправным. Смешанные ошибки не считаются фатальными.
...
Закреплённый Xray передаёт через локальный Unix datagram-сокет в `/run` только
PID, tag и стадию ошибки соединения с VLESS-сервером. Фоновые пробы не создают
сигнал; ошибки сайтов после подключения к серверу также не считаются его
отказом. Health-agent принимает сигнал лишь от текущего процесса и выбранного
обработчика, ограничивает повторы и немедленно запускает обычную HTTPS-проверку
активного узла, в том числе во время фонового замера. Сигнал не меняет selector
и не отменяет проверку WAN/DNS или резерва. При недоступном сокете остаётся
прежний периодический опрос.
`connection refused`, `no route` и фатальная TLS-ошибка всех контрольных целей
подтверждают отказ сразу; обычный timeout требует повторной проверки в следующем
fast-lane цикле (по умолчанию через 2 секунды), остальные ошибки — настроенного
порога. Перед сменой узла общий sentinel проверяет прямой WAN и DNS:
...
резерв из health-карты без новой сетевой проверки. Если его нет, настроенное
число изолированных availability-линий (Auto: 2/3, вручную: до 10) проверяет
аварийную порцию параллельно. Первый подходящий результат немедленно переключает
selector. Незавершённые пробы этой порции отменяются; фоновый мониторинг
кандидатов продолжается в следующих циклах. После аварийного перехода рабочий резерв
...
подтверждённый отказ доступности всё равно переключает немедленно. Возврат на
более приоритетный узел защищён такой же паузой от обратного переключения по
quality-ошибкам. Первичный
...
Перед аварийной порцией ограниченный трёхсекундный TCP-отбор по всему списку
выдвигает доступные числовые IP-адреса VLESS вперёд независимо от размера
листа. Закрытый порт пропускается в этой порции, но перепроверяется позднее;
домен, UDP и тайм-аут остаются на обычном пути. Открытый порт не означает
работающий узел: переключение разрешает только свежая HTTPS-проба через Xray.
Фоновые команды проверки имеют отдельный ограниченный пул, чтобы подготовка
резервов не удерживала слот рабочего переключения. При смене фактического
outbound с прежним ID прежняя история доступности этого узла сбрасывается;
косметическое переименование её не меняет.
Когда selector находится в `block`, health-loop перепроверяет ранее рабочие
узлы с быстрым интервалом (по умолчанию 2 секунды), а остальные — с интервалом
повторного обхода (по умолчанию 15 секунд). В режиме Auto порция ограничена
двумя кандидатами URLTest или тремя кандидатами приоритетного листа; вручную
можно выбрать до 10. Размер порции не изменяет рабочий пул URLTest. Сохранённое
время последнего успешного probe помогает начать с вероятного рабочего пути;
быстрые места чередуют ранее работавшие узлы. При порции из 10 проверок им
отводятся три места, а семь продвигают обход всего pool, включая узлы вне
shortlist. Выбор возможен только после свежей
HTTPS-пробы, а не по старой истории. Первая свежая HTTPS-доступность
...
удаления. Перед подтверждением выбранного динамического узла и перед повторным
использованием handler агент читает `HandlerService/ListOutbounds`; если tag
исчез при сохранённом selector override, handler создаётся заново и
подтверждается повторным чтением. Та же сверка выполняется при восстановлении
из локального кэша, даже если новый health-контракт ещё не загружен. Ошибка восстановления оставляет runtime
неподтверждённым. Новый поток сразу идёт через новую generation.
