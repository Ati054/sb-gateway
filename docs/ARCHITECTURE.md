...
первая контрольная цель не ответила, быстрая проверка пробует вторую независимую
HTTPS-цель: отказ одного сайта не объявляет узел неисправным. Timeout каждой
цели ограничен 2 секундами; смешанные ошибки не считаются фатальными.
...
`connection refused`, `no route` и фатальная TLS-ошибка двух контрольных целей
